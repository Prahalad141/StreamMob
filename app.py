...
if location != "Select":
    ...
    smart_slot = next(
        (i for i in range(total_slots)
         if i >= area["booked"] and not st.session_state.booked_slots.get(f"{location}-{i}", False)),
        None
    )
    ...
    map_center = area_coords[location]
    m = folium.Map(location=map_center, zoom_start=14)
    folium.Marker(location=map_center, popup=location, tooltip="Center Location", icon=folium.Icon(color="red")).add_to(m)
    ...
    bar_fig.update_layout(title="🚦 Slot Status across Areas", barmode='group')
    st.plotly_chart(bar_fig, use_container_width=True)

    # 3D Sub-Place Popularity Chart
    ...
    fig3d.update_layout(scene=dict(xaxis_title='Area', yaxis_title='Sub Place', zaxis_title='Popularity'),
                        title="📍 Sub-Places Popularity View (3D)")
    ...
    for i in range(total_slots):
        ...
        with cols[i % 5]:
            if not is_booked:
                ...
                st.markdown(f"<div style='background-color:red; color:white; padding:10px; text-align:center; border-radius:5px;'>{label} (Booked)</div>", unsafe_allow_html=True)

    # 3D Slot Visualization
    ...
    slot_fig = go.Figure(data=[go.Scatter3d(
        ...
        z=[1]*len(df_slots),
        mode='markers+text',
        marker=dict(size=12, color=[c for c in df_slots["Color"]]),
        text=df_slots["Slot"],
        textposition="top center"
    )])
    slot_fig.update_layout(scene=dict(xaxis_title="Column", yaxis_title="Row", zaxis_title="Level"),
